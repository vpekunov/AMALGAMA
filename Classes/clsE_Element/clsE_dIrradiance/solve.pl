...
expand_irradiances(ID,Class,Name,Image,ColorI,ColorO,ColorD,PhaseID,PhaseCID,SolverID,SolverCID,LeftBase,TopBase):-
    ...
    (
     (
      predicate_property(temperature(_),'dynamic'), temperature(TID) ->
       str_replace('"double TK = 273.15+$2;","double $1kAlbAsm = $1kAlbedo*$1kAsymm;","double $1tAlpha = $1eBeta*(1-$1kAlbedo);","double $1tBeta = $1eBeta*(1-$1kAlbAsm);","double $1Planck = (@[]IntPlanck[0]+TK*(@IntPlanck[1]+TK*(@IntPlanck[2]+TK*@IntPlanck[3])));","double $1Ft = 3.0*$1tAlpha*$1Planck;"',[Prefix,TID],VarsK);
       str_replace('"double TK = 273.15+DefaultT;","double $1kAlbAsm = $1kAlbedo*$1kAsymm;","double $1tAlpha = $1eBeta*(1-$1kAlbedo);","double $1tBeta = $1eBeta*(1-$1kAlbAsm);","double $1Planck = (@[]IntPlanck[0]+TK*(@IntPlanck[1]+TK*(@IntPlanck[2]+TK*@IntPlanck[3])));","double $1Ft = 3.0*$1tAlpha*$1Planck;"',[Prefix],VarsK)
     ),
     str_replace('"Split{0.0,-$1Ft*$1tBeta};"',[Prefix],ExprK),
     str_replace('"double $1k1 = 2.0/3.0/$1tBeta;","double $1k1a = $1k1*(2.0-@SurfaceEmissivity);","double $1FluxPi = @$1DiffuseFlux/Pi;"',[Prefix],VarsB),
     str_replace('"      if (IsClosed($2))","        {","         if (IsForw)   _fw($2) = ($1k1a*$2[back]+HY[y-1]*@SurfaceEmissivity*(@[]IntPlanck[0]+(273.15+_fw($3))*(@IntPlanck[1]+(273.15+_fw($3))*(@IntPlanck[2]+(273.15+_fw($3))*@IntPlanck[3])))/Pi)/($1k1a+HY[y-1]*@SurfaceEmissivity);","         if (IsBack)   _bw($2) = ($1k1a*$2[forw]+HY[y]*  @SurfaceEmissivity*(@[]IntPlanck[0]+(273.15+_bw($3))*(@IntPlanck[1]+(273.15+_bw($3))*(@IntPlanck[2]+(273.15+_bw($3))*@IntPlanck[3])))/Pi)/($1k1a+HY[y]*@SurfaceEmissivity);","         if (IsBottom) _bt($2) = ($1k1a*$2[up]+HZ[z]*    @SurfaceEmissivity*(@[]IntPlanck[0]+(273.15+_bt($3))*(@IntPlanck[1]+(273.15+_bt($3))*(@IntPlanck[2]+(273.15+_bt($3))*@IntPlanck[3])))/Pi)/($1k1a+HZ[z]*@SurfaceEmissivity);","         if (IsTop)    _tp($2) = ($1k1a*$2[down]+HZ_ZM*  @SurfaceEmissivity*(@[]IntPlanck[0]+(273.15+_tp($3))*(@IntPlanck[1]+(273.15+_tp($3))*(@IntPlanck[2]+(273.15+_tp($3))*@IntPlanck[3])))/Pi)/($1k1a+HZ_ZM*@SurfaceEmissivity);","         if (IsLeft)   _lf($2) = ($1k1a*$2[right]+HX[x]* @SurfaceEmissivity*(@[]IntPlanck[0]+(273.15+_lf($3))*(@IntPlanck[1]+(273.15+_lf($3))*(@IntPlanck[2]+(273.15+_lf($3))*@IntPlanck[3])))/Pi)/($1k1a+HX[x]*@SurfaceEmissivity);","         if (IsRight)  _rg($2) = ($1k1a*$2[left]+HX[x-1]*@SurfaceEmissivity*(@[]IntPlanck[0]+(273.15+_rg($3))*(@IntPlanck[1]+(273.15+_rg($3))*(@IntPlanck[2]+(273.15+_rg($3))*@IntPlanck[3])))/Pi)/($1k1a+HX[x-1]*@SurfaceEmissivity);","        }","      else","        {","         if (IsForw)   _fw($2) = ($1k1*$2[back]+HY[y-1]*$1FluxPi)/($1k1+HY[y-1]);","         if (IsBack)   _bw($2) = ($1k1*$2[forw]+HY[y]*$1FluxPi)/($1k1+HY[y]);","         if (IsBottom) _bt($2) = ($1k1*$2[up]+HZ[z]*$1FluxPi)/($1k1+HZ[z]);","         if (IsTop)    _tp($2) = ($1k1*$2[down]+HZ_ZM*$1FluxPi)/($1k1+HZ_ZM);","         if (IsLeft)   _lf($2) = ($1k1*$2[right]+HX[x]*$1FluxPi)/($1k1+HX[x]);","         if (IsRight)  _rg($2) = ($1k1*$2[left]+HX[x-1]*$1FluxPi)/($1k1+HX[x-1]);","        }"',[Prefix,VarID,TID],ExprB)
    );
    (
     str_replace('"double Pi = 3.1415926535897932;","double $1kAlbAsm = $1kAlbedo*$1kAsymm;","double $1tAlpha = $1eBeta*(1-$1kAlbedo);","double $1tBeta = $1eBeta*(1-$1kAlbAsm);","double $1_Ft = 3.0*$1eBeta*$1kAlbedo/4.0/Pi;","double $1Ft = $1_Ft*$2;"',[Prefix,VarFID],VarsK),
     str_replace('"Split{0.0,-$1Ft*$1tBeta};"',[Prefix],ExprK),
     str_replace('"double $1k1 = 2.0/3.0/$1tBeta;","double $1k1a = $1k1*(1+@SurfaceAlbedo);","double $1FluxPi = @$1DiffuseFlux/Pi;"',[Prefix],VarsB),
     str_replace('"      if (IsClosed($2))","        {","         if (IsForw)   _fw($2) = ($1k1a*$2[back]+HY[y-1]*SolarY*_fw($3)*@SurfaceAlbedo/Pi)/($1k1a+HY[y-1]*(1-@SurfaceAlbedo));","         if (IsBack)   _bw($2) = ($1k1a*$2[forw]-HY[y]*SolarY*  _bw($3)*@SurfaceAlbedo/Pi)/($1k1a+HY[y]*(1-@SurfaceAlbedo));","         if (IsBottom) _bt($2) = ($1k1a*$2[up]-HZ[z]*SolarZ*  _bt($3)*@SurfaceAlbedo/Pi)/($1k1a+HZ[z]*(1-@SurfaceAlbedo));","         if (IsTop)    _tp($2) = ($1k1a*$2[down]+HZ_ZM*SolarZ*  _tp($3)*@SurfaceAlbedo/Pi)/($1k1a+HZ_ZM*(1-@SurfaceAlbedo));","         if (IsLeft)   _lf($2) = ($1k1a*$2[right]-HX[x]*SolarX*  _lf($3)*@SurfaceAlbedo/Pi)/($1k1a+HX[x]*(1-@SurfaceAlbedo));","         if (IsRight)  _rg($2) = ($1k1a*$2[left]+HX[x-1]*SolarX*_rg($3)*@SurfaceAlbedo/Pi)/($1k1a+HX[x-1]*(1-@SurfaceAlbedo));","        }","      else","        {","         if (IsForw)   _fw($2) = ($1k1*$2[back]+HY[y-1]*$1FluxPi)/($1k1+HY[y-1]);","         if (IsBack)   _bw($2) = ($1k1*$2[forw]+HY[y]*$1FluxPi)/($1k1+HY[y]);","         if (IsBottom) _bt($2) = ($1k1*$2[up]+HZ[z]*$1FluxPi)/($1k1+HZ[z]);","         if (IsTop)    _tp($2) = ($1k1*$2[down]+HZ_ZM*$1FluxPi)/($1k1+HZ_ZM);","         if (IsLeft)   _lf($2) = ($1k1*$2[right]+HX[x]*$1FluxPi)/($1k1+HX[x]);","         if (IsRight)  _rg($2) = ($1k1*$2[left]+HX[x-1]*$1FluxPi)/($1k1+HX[x-1]);","        }"',[Prefix,VarID,VarFID],ExprB),
     TopF is Top+110,
  ...
  (predicate_property(temperature(_),'dynamic'),temperature(TID) ->
    (
     ...
     (
      predicate_property(thermal(_,_),'dynamic'),thermal(ID,RangeName) ->
       str_replace('"Split{0.0,4.0*Pi*($1tAlpha*$2-$1Ft/3.0)/RO0/CC0};"',[Prefix,VarID],ExprKT);
       str_replace('"Split{0.0,$1tAlpha*(4.0*Pi*$2+$3)/RO0/CC0};"',[Prefix,VarID,VarFID],ExprKT)
     ),
  ...
  str_replace('"Split{0.0,3.0*$1tAlpha*$1tBeta};"',[Prefix],ExprS),
